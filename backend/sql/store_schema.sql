...
CREATE TABLE `budgets` (
  ...
  `target_sales` decimal(20,2) DEFAULT 0.00,
  `target_profit` decimal(20,2) DEFAULT 0.00,
  `max_expenses` decimal(20,2) DEFAULT 0.00,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
...
CREATE TABLE `customer_analytics` (
  ...
  `payment_reliability_score` decimal(3,2) DEFAULT 1.00,
  `days_since_last_purchase` int(11) DEFAULT 0,
  `purchase_frequency_days` decimal(8,2) DEFAULT 0.00,
...
CREATE TABLE `debts` (
  ...
  `tenant_id` int(11) NOT NULL,
  `shop_id` int(11) NOT NULL,
  `transaction_id` int(11) DEFAULT NULL COMMENT 'Links to original POS transaction',
  ...
  `recorded_by` int(11) NOT NULL COMMENT 'User ID of cashier who created debt',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
...
CREATE TABLE `debt_payments` (
  ...
  `recorded_by` int(11) NOT NULL COMMENT 'User ID of cashier who recorded payment',
  `notes` text DEFAULT NULL COMMENT 'Optional payment notes',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
...
CREATE TABLE `expenses` (
  ...
  `category` enum('rent','salaries','utilities','marketing','repairs','supplies','transportation','other') DEFAULT 'other',
  `date` date NOT NULL,
...
CREATE TABLE `expense_records` (
  ...
  `daily_expenses` decimal(20,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE `inventory` (
  ...
  `status` enum('in_stock','sold','returned','in_transit') DEFAULT 'in_stock',
...
CREATE TABLE `marketplace_messages` (
  ...
  `message_type` enum('text','listing_card','product_card','order_card','image','system') DEFAULT 'text',
...
CREATE TABLE `profit_records` (
  ...
  `daily_profit` decimal(20,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE `reports` (
  ...
  `operating_profit` decimal(20,2) DEFAULT 0.00,
...
CREATE TABLE `transaction_items` (
  ...
  `price` decimal(20,2) NOT NULL,
  `type` enum('sale','trade_in','manual') NOT NULL,
  `description` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `vendors` (
  `id` int(11) NOT NULL,
  `tenant_id` int(11) NOT NULL DEFAULT 1 COMMENT 'Parent business/tenant',
  `shop_id` int(11) NOT NULL COMMENT 'Shop ID',
  `name` varchar(100) NOT NULL COMMENT 'Vendor Name',
  `address` text DEFAULT NULL COMMENT 'Vendor Address',
  `contact_info` varchar(255) DEFAULT NULL COMMENT 'Email or Phone',
...
ALTER TABLE `budgets`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_budget` (`tenant_id`,`shop_id`,`budget_month`);
...
ALTER TABLE `customer_analytics`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_customer_shop` (`customer_phone`,`shop_id`),
  ADD KEY `idx_segment` (`segment`),
  ADD KEY `idx_ltv` (`lifetime_value`),
...
ALTER TABLE `marketplace_interests`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `idx_user_listing_shop` (`user_id`,`listing_id`,`shop_id`),
  ADD KEY `listing_id` (`listing_id`),
  ADD KEY `idx_user_fk_restore` (`user_id`);
...
ALTER TABLE `order_disputes`
  ...
  ADD KEY `idx_order_dispute_shop` (`shop_id`);
...
ALTER TABLE `vendors`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_vendors_shop` (`shop_id`),
  ADD KEY `idx_vendors_tenant` (`tenant_id`);
...
--
-- Constraints for table `customer_analytics`
--
ALTER TABLE `customer_analytics`
  ADD CONSTRAINT `customer_analytics_ibfk_1` FOREIGN KEY (`shop_id`) REFERENCES `shops` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `customer_analytics_ibfk_2` FOREIGN KEY (`tenant_id`) REFERENCES `tenants` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `debts`
  ...
  ADD CONSTRAINT `debts_ibfk_2` FOREIGN KEY (`transaction_id`) REFERENCES `transactions` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `debts_ibfk_3` FOREIGN KEY (`recorded_by`) REFERENCES `users` (`id`);
...
ALTER TABLE `debt_payments`
  ...
  ADD CONSTRAINT `debt_payments_ibfk_2` FOREIGN KEY (`recorded_by`) REFERENCES `users` (`id`);
